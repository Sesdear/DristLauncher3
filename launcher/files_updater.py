...
def updateFiles(upd_dict: dict, progress_callback=None):
    ...
    for i in upd_dict["download"]:
        ...
        try:
            ...
            if i.endswith(".zip"):
                extract_dir = os.path.join(".", i[:-4])  # Путь для извлечения
                os.makedirs(extract_dir, exist_ok=True)  # Создаем директорию, если её нет
                try:
                    ...
                    with zipfile.ZipFile(filepath, 'r') as zip_ref:
                        ...
                        # Filter the files to only those that are in the specified folders
                        for file in all_files:
                            # Extract folder names from the file path
                            folder_name = file.split('/')[0]

                            if folder_name in ['kubejs', 'fancymenu']:
                                # Extract only the files that match the specified folders
                                zip_ref.extract(file, 'minecraft')
                                print(f"Extracted '{file}' to '")
                except Exception as e:
